    ...
    internal class Program
    {
        ...
        static void Crud()
        {
            ...
            while (x)
            {
                ...
                Console.WriteLine("------------------------------------------------");
                Console.WriteLine("Press " + $"{(int)(Utility.Op.ExportDatatoExcel)}" + " for viweing data in Excel Table");
                Console.WriteLine("Press " + $"{(int)(Utility.Op.ExportDatatoCSV)}" + " for viweing data in CSV Table");
                ...
                switch (N)
                {
                    ...
                    case (int)(Utility.Op.ExportDatatoCSV):
                        DetailsCSV();
                        break;
                    case (int)(Utility.Op.ExportDatatoExcel):
                        DetailsExcel();
                        break;
                    default:
                        x = false;
        ...
        private static void DetailsCSV()
        {
            Console.WriteLine("Select table for showing data");
            Console.WriteLine("Press 1 for Employee");
            Console.WriteLine("Press 2 for Branch");
            Console.WriteLine("Press 3 for Department");
            Console.WriteLine("Press 4 for Combined Data");

            Console.Write("\nInput your choice :");
            int N = Convert.ToInt32(Console.ReadLine());
            string fileName1 = @"D:\MINDFIRE\source\repos\EmployeeLayers\output\Dataoutput.csv";

            if (N == 1)
            {
                List<EmployeeInsert> list = BusinessLayer.ViewEmployee(fileName);
                BusinessLayer.ExportDataToCsv(list, fileName1);
            }
           else  if(N == 2) {
                List<BranchInsert> list = BusinessLayer.ViewBranch(fileName);
                BusinessLayer.ExportDataToCsv(list, fileName1);
            }
           else if (N == 3)
            {
                List<DepartmentInsert> list = BusinessLayer.ViewDepartment(fileName);
                BusinessLayer.ExportDataToCsv(list, fileName1);
            }
            else if (N == 4)
            {
                List<CombinedData> list = BusinessLayer.GetCombinedDatas(fileName);
                BusinessLayer.ExportDataToCsv(list, fileName1);
            }
            else
            {
        ...
        private static void DetailsExcel()
        {
            Console.WriteLine("Select table for showing data");
            Console.WriteLine("Press 1 for Employee");
            Console.WriteLine("Press 2 for Branch");
            ...
            Console.Write("\nInput your choice :");
            int N = Convert.ToInt32(Console.ReadLine());
            string fileName1 = @"D:\MINDFIRE\source\repos\EmployeeLayers\output\Datalist.xlsx";

            if (N == 1)
            {
                List<EmployeeInsert> list = BusinessLayer.ViewEmployee(fileName);
                BusinessLayer.ExportDataToExcel(list, fileName1);
            }
            else if (N == 2)
            {
                List<BranchInsert> list = BusinessLayer.ViewBranch(fileName);
